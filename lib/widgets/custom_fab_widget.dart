...
class _CustomFABWidgetState extends State<CustomFABWidget>
  ...
  void initState() {
    ...
    controller =
        AnimationController(vsync: this, duration: widget.duration ?? const Duration(milliseconds: 350));
  ...
  void paintChildren(FlowPaintingContext context) {
    ...
    if (n <= 3) {
      ...
      for(int i = 0 ; i < n ; i++){
        dynamic x = 0;
        dynamic y = 0 ;
        if(currentPosition == Position.TopLeft || currentPosition == Position.BottomLeft) {
          ...
          dynamic rad = (n <= 2) ?  2*pi /3 - (n - i) * 2 * pi / 6 :  pi / 3 - (n - 1 - i) * 2 * pi / 6;

          x = currentMainButtonOffset!.dx - (radius * cos(rad));
          y = (n > 2 )? currentMainButtonOffset!.dy + (radius * sin(rad)) : currentMainButtonOffset!.dy - (radius * sin(rad)) ;
      ...
      for (var i = 0; i < n; i++) {
        ...
        context.paintChild(i,
            transform: Matrix4.identity()
              ..translate(x, y, 0)
              ..translate(buttonSize! / 2, buttonSize! / 2)
              ..rotateZ((degree * (1 - animationController.value) * pi / degree))
              ..translate(-buttonSize! / 2, -buttonSize! / 2));
      }
    }
  }
